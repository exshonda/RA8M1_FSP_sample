...
void hal_entry(void) {
    ...
    R_IOPORT_PinWrite(&g_ioport_ctrl, BSP_IO_PORT_06_PIN_03, BSP_IO_LEVEL_LOW);
    R_IOPORT_PinWrite(&g_ioport_ctrl, BSP_IO_PORT_06_PIN_04, BSP_IO_LEVEL_LOW);
     while (true)
     {
         ...
         R_IOPORT_PinWrite(&g_ioport_ctrl, BSP_IO_PORT_06_PIN_03, BSP_IO_LEVEL_HIGH);
         R_BSP_SoftwareDelay(500, BSP_DELAY_UNITS_MILLISECONDS);
     }
...
void g_external_irq13_callback(external_irq_callback_args_t *p_args)
{
    ...
    if (gled_on == 0) {
       led_set_guard(g_bsp_leds.p_leds[1], BSP_IO_LEVEL_HIGH);
        gled_on = 1;
    } else {
        led_set_guard(g_bsp_leds.p_leds[1], BSP_IO_LEVEL_LOW);
        gled_on = 0;
    }
    R_IOPORT_PinWrite(&g_ioport_ctrl, BSP_IO_PORT_06_PIN_04, BSP_IO_LEVEL_LOW);
}
